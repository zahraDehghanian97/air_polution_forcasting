...
def write2db(db, date_list, stationID, station_name, predicted_values, real_values):
    ...
    for i in range(len(predicted_values)):
        ...
        jalili = data_helper.convert_to_jalili_date(current_date)

        date = str(current_date).split()[0]
        persianDate = jalili

        real_AQI = 0  # real_values[0]
        if np.isnan(real_AQI):
            real_AQI = 'Null'

        real_AQI_p = real_values[0]
        if np.isnan(real_AQI_p):
            real_AQI = 'Null'
        ...
        # if i == 0:
        #     query = "update dbo.AQI_real_predict set real_AQI = " + str(real_AQI_p) + \
        #             " where date = '" + str(previous_date) + "' and stationID = " + str(stationID)
        if i < len(predicted_values)-1:  # and False:
            query = "update dbo.AQI_real_predict set predict_AQI = " + str(predict_AQI) + \
                    " where date = '" + str(date) + "' and stationID = " + str(stationID)
        else:
...
if __name__ == "__main__":
    ...
    # model_list = ["ARIMAXW", "ARIMAXH", "ARIMA", "ES"]
    ...
    for station in station_list:
        ...
        # mydb = None
        write2db(db=mydb, date_list=[date_list[-1]], stationID=station_id, station_name=station_name,
                 predicted_values=np.squeeze(y_pred)[-1], real_values=np.squeeze(y_true)[-2])
